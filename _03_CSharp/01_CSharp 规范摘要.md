...
查询表达式用于查询和转换启用 LINQ 数据源中的数据。`IEnumerable`、`IEnumerable<T>` 或 `IQueryable`、`IQueryable<T>` 称为可查询类型。`IEnumerable<T>` 查询编译为委托。`IQueryable<T>` 查询编译为表达式树。
...
// 编译器生成的异步状态机
private struct CalculateSumAsyncStateMachine : IAsyncStateMachine
{
    public int _state;  // 状态标记，-1:未执行，0:首次执行，1:await后恢复执行
    private TaskAwaiter _awaiter;  // 异步任务Awaiter，用于等待异步操作完成或挂起执行
    // 保存方法参数、局部变量
    private int _a;
    private int _b;
    private int _temp;
    ...
    public void MoveNext()
    {
        ...
        try
        {
            ...
            if (_state != 1)
            {
                // 首次执行：同步代码片段1
                _temp = _a + _b;
                // 执行 await Task.Delay，获取Awaiter
                _awaiter = Task.Delay(100).GetAwaiter();
                // 判断任务是否完成
                if (!_awaiter.IsCompleted)
                {
                    // 任务未完成：设置状态，注册回调，挂起返回
                    _state = 1;
                    // 注册回调：任务完成后调用 MoveNext 恢复
                    _awaiter.OnCompleted(MoveNext);
                    return;
                }
            }
            // 任务完成/恢复执行：同步代码片段 2（await 后续）
            _state = -1;
            _awaiter.GetResult(); // 获取任务结果，抛出异常（如有）
            result = _temp * 2;
        }
        catch (Exception ex)
        {
