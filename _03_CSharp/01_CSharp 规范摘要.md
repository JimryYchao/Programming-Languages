...
C# 组织结构中包括程序、命名空间、类型、成员和程序集（`*.exe` 或 `*.dll`）。程序集包含中间语言（IL）指令的可执行代码和元数据符号信息。加载程序集时，CLR 运行时 JIT 即时编译器将 IL 代码编译为本机代码。
...
`Main` 可选包含 `string[] args`。程序名称不是 `args` 首元，是 `Environment.GetCommandLineArgs()` 首元。
...
在单独一个编译单元以顶级语句替代 `Main`，可以使用 `await` 和 `return`，编译器为其自动生成正确的 `Main`。

```csharp
// Program.cs
global using System;
Console.WriteLine("Hello World! {0}", Environment.GetCommandLineArgs()[0]);
// 顶级语句中，编译器为其生成正确的 Main。
...
| :----------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 类型声明           | `void`,`bool`,`char`,`string`,`object`,`dynamic`<br>`sbyte`,`short`,`int`,`long`,`byte`,`ushort`,`uint`,`ulong`<br>`nint`,`nuint`,`float`,`double`,`decimal`,<br>`enum`,`struct`,`class`,`record`,`interface`,`delegate`   |
| 文本值             | `null`,`true`,`false`                                                                                                                                                                                                      |
| 命名空间           | `namespace`,`using`,`global`,`using static`,`extern alias`                                                                                                                                                                 |
| 泛型约束           | `where`,`new`,`default`,`notnull`,`unmanaged`,`allows ref struct`                                                                                                                                                          |
| 访问性限制         | `private`,`protected`,`public`,`internal`,`file`                                                                                                                                                                           |
| 声明修饰符         | `abstract`,`override`,`virtual`,`sealed`,`event`,`record`,`async`,`require`<br>`const`,`volatile`,`readonly`,`event`,`extern`,`unsafe`,<br>`in`(逆变),`new`(成员),`out`(协变),`scoped`(作用域)                             |
| 参数修饰符         | `ref`,`out`,`ref readonly`,`in`,`params`                                                                                                                                                                                符 |
| 访问关键字         | `base`,`this`                                                                                                                                                                                                              |
| 访问器             | `add`,`remove`,`get`,`set`,`value`,`field`,`init`                                                                                                                                                                          |
| 用户定义运算和转换 | `operator`,`explicit`,`implicit`                                                                                                                                                                                           |
| 表达式             | `new`,`stackalloc`,`default`,`sizeof`,`typeof`,`nameof`,<br>`is`,`as`,`await`,`with`                                                                                                                                       |
| 模式匹配与类型推断 | `var`,`is`,`switch`,`and`,`or`,`not`.`_` 弃元                                                                                                                                                                              |
| 迭代语句           | `while`,`do`,`for`,`foreach`                                                                                                                                                                                               |
| 条件语句           | `if`,`else`,`switch`,`case`,`default`,`when`                                                                                                                                                                               |
| 跳转语句           | `goto`,`continue`,`break`,`return`                                                                                                                                                                                         |
| 异常处理           | `throw`,`try`,`catch`,`finally`,`when`                                                                                                                                                                                     |
| 溢出检查           | `checked`,`unchecked`                                                                                                                                                                                                      |
| 其他语句           | `using`,`lock`,`fixed`,`yield`                                                                                                                                                                                             |
| 成员拓展           | `extension`                                                                                                                                                                                                                |
| 查询关键字         | `from`,`where`,`select`,`group`,`into`,`orderby`,`join`,<br>`let`,`in`,`on`,`equals`,`by`,`ascending`,`descending`                                                                                                         |
...
C# 是强类型语言，包含值类型（*stack*）和引用类型（*heap*）。值类型包括简单类型、枚举、结构、元组和可空值类型；引用类型包括字符串、类、接口、数组和委托类型。

内置类型包括 `bool`,`char`,`byte`,`short`,`int`,`long`,`sbyte`,`ushort`,`uint`,`ulong`,`nint`,`nuint`,`float`,`double`,`decimal`,`object`,`string`。用户定义类型包括 `class`,`struct`,`interface`,`enum`,`delegate`,`record struct`,`record class`。

用户定义类型具有成员，可以是静态或实例成员：
...
非托管类型（`unmanaged` 约束）：`sbyte`、`byte`、`short`、`ushort`、`int`、`uint`、`long`、`ulong`、`nint`、`nuint`、`char`、`float`、`double`、`decimal`、`bool`、枚举、指针、非托管结构类型。托管（*managed*）对象堆上分配，变量保存其引用。CLR 自动执行内存管理和垃圾回收。
...
所有类型（除指针和 `ref struct` 外）的通用基类是 `object`。*装箱和拆箱* 提供值类型和引用类型之间互相转换的桥梁。

装箱是值类型到引用类型的隐式转换，拆箱是从引用类型提取值到值类型的显式转换。`is` 测试装箱类型。
...
| 成员所属    | 默认访问性 | 可声明访问性                                                                            |
| :---------- | :--------- | :-------------------------------------------------------------------------------------- |
| 顶级类型    | `internal` | `internal`、`public`、`file`                                                            |
...
class Derived : Base {
    protected new double Value { get; set; }   // 隐藏 base.Value
}
```
...
命名空间 `namespace` 对程序元素进行逻辑分组，`global::` 表示全局访问。
...
`using` 引入命名空间；`using static` 导入类型成员；`using Id = T` 创建别名；`global using` 表示全局指令。

```csharp
global using System;                      // 全局引入
global using Generic = System.Collections.Generic;    // 命名空间别名
using static System.Math;                 // 静态导入成员
...
`extern alias` 外部引用程序集别名，以区别多个程序集中具有相同完全限定名称的程序元素。例如在项目设置中导入 `grid.dll` 和 `grid20.dll` 的引用并创建别名为 “GridV1” 和 “GridV2”：
...
3.14m,2.71M           // decimal
1.2345E15,9.8e2;      // E 计数法
```

`decimal` 表示 128 位十进制数，可以精确表示 0.1，与标准浮点不存在隐式转换，过大过小的浮点运算会引发溢出异常，标准浮点运算不会产生异常。
...
枚举基础类型默认为 `int`，可显式转换。其他类型可以是 `byte`、`sbyte`、`short`、`ushort`、`int`、`uint`、`long`、`ulong`。
...
在 `struct`、`class`、`record`、`interface` 中声明解构函数 `Deconstruct` 或扩展方法，拥有至少两个 `out` 参数，可重载。主构造函数记录自动合成 `Deconstruct` 方法。
...
`Dictionary` 的元素 `KeyValuePair<TKey,TValue>`，内置解构函数。
...
`$string` 的内插表达式（`{expr}`）替换为表达式结果（`ToString`）的字符串形式。`{{` 和 `}}` 转义为 `"{"` 和 `"}"`。
...
数组是单一类型元素的编号序列。多维数组的每个维度都有一个相关联的长度（秩）。
...
readonly struct Sample(string first, string last) {
    public readonly string FSDArstName = first;
...
`ref struct` 在堆栈（*stack*）上分配：
  - 可以实现接口，或作为类型参数（`allows ref struct`）。
  - 非 `class` 成员、数组、元组元素。
  - 在异步方法和异步迭代器中，访问局部变量的 *ref-like* 不能跨 `await` 边界。

`ref` 字段只在 `ref struct`，默认为 `null`。`readonly` 限定 `ref` 字段：
- `readonly ref` 引用只读
- `ref readonly` 引用关联值只读
- `readonly ref readonly` 引用和关联值只读
...
`fixed` 在结构中创建固定大小缓冲区，类型为 `bool`、`byte`、`char`、`short`、`int`、`long`、`sbyte`、`ushort`、`uint`、`ulong`、`float` 或 `double`。
...
内联数组（`InlineArrayAttribute`）是 *N* 个具有唯一非指针字段的结构类型组成的连续块，支持范围和索引运算。
...
`class` 支持单一继承。NET 一些预定义类在 C# 中具有特殊含义：
- `System.Object`：最终基类。
- `System.String`：字符串类型。
...
`abstract` 声明抽象类、纯虚函数类成员；`virtual` 声明虚函数类成员；派生类 `override` 实现或重写虚方法；`sealed` 封装类防止继承，或封装函数防止重写。
...
`static class` 仅包含静态成员，静态类全局生存；扩展方法或扩展成员在静态类中声明：
...
委托用于封装一组方法调用列表，委托调用转换为绑定方法组的调用序列。在构造委托时，编译器会为委托生成 `Invoke`，Framework 额外生成异步调用 `BeginInvoke`、`EndInvoke`。`+` / `+=` 添加方法或委托；`-` / `-=` 用于删除组件（优先匹配最后一个），无法删除匿名函数。

```csharp
// 委托的定义
delegate <TResult> SampleDelegate(<ParamsList>);
// 编译器为委托类型生成
sealed class SampleDelegate : System.MulticastDelegate
{
    public SampleDelegate(object obj, IntPtr method);
    public virtual <TResult> Invoke(<ParamsList>);
    public virtual IAsyncResult BeginInvoke(<ParamsList>, AsyncCallback callback, object obj);
    public virtual <TResult> EndInvoke([out <Params> /*from ParamsList*/,]? IAsyncResult asyncResult);
}
```

预定义委托类型包含 `Action`、`Func`、`Predicate` 。
...
在异步模式下，委托调用方继续执行而不等待委托返回。`BeginInvoke` 和 `EndInvoke` 执行委托异步调用。异步产生的异常通过 `EndInvoke` 传播到调用方。
...
通过两种方式来等待异步计算结果：
- 一是通过 `BeginInvoke` 结果对象 `IAsyncResult` 作为 `EndInvoke` 参数检查异步结果。
...
- 二是通过传递给 `BeginInvoke` 的 `AsyncCallback` 对象作为异步回调。`AsyncCallback` 的参数与 `BeginInvoke` 返回值相同。
...
匿名函数（`delegate` 表达式或 Lambda）可以转换为委托或表达式树。支持 `static`、`async`、`scoped`、弃元（两个参数以上）、函数闭包。
...
`volatile` 声明易变字段；`readonly` 只读字段在初始化器、构造函数或 `init` 中初始化；`required` 声明必填字段；`ref` 声明引用字段（`ref struct`）。
...
`required` 字段要求在创建实例时初始化。`required` 类型不满足 `new()` 约束。
...
方法支持重载和继承，可以按引用 `ref` / `ref readonly` 返回。`readonly` 只读方法仅在结构中声明，表明不修改结构实例状态。
...
`params` **可变参数** 可以是数组或集合类型。用户定义类型需要包含一个 `Add` 方法。

```csharp
class MList<T>: IEnumerable<T> {
    private System.Collections.Generic.List<T> datas = new();
    public IEnumerator<T> GetEnumerator() => datas.GetEnumerator();
    public void Add(T v) => datas.Add(v);
    IEnumerator IEnumerable.GetEnumerator() => GetEnumerator();
}
class Sample {
    static void PrintList<T>(params MList<T> args) {
        if (args != null)
...
**可选参数** 具有默认值，位于一般参数之后。`OptionalAttribute` 特性参数声明可选且无需默认值；`DefaultParameterValueAttribute` 为 `OptionalAttribute` 参数设置默认值。
...
位置参数按顺序传递；**命名参数** 任意顺序传递，位于位置实参之后。
...
引用参数可以设置作用域修饰   `scoped` 或 `[UnscopedRef]`，限制引用参数的作用域范围为 *current method*、*calling method* 或 *return only*。例如 `scoped ref value` 参数无法通过 `return ref` 返回给调用方，它的安全转义限定为 *current method*。
...
方法支持重载，编译器利用重载决策来匹配最优。参数 `object` / `dynamic` 和 `T[]` / `params T[]` 被视为是相同的。
...
在块范围内声明局部函数，无法重载。局部方法可以是 `async`、`unsafe`、`static`、`extern static`。`Conditional` 特性标记的局部方法只能是 `static`。
...
`async` 异步方法返回类型为 `Task`、`Task<R>`、`ValueTask`、`ValueTask<R>`。启动异步或异步事件的 `async` 方法可以返回 `void`。

```csharp
async Task<TResult> FuncAsync(){
    PreOperation();
    // 异步阻塞等待 LongTimeOperationAsync 完成
    TResult result = await LongTimeOperationAsync();
    PostOperation(result);
    return result;
}
async Task FuncVoidAsync(){
    // ...
    await LongTimeOperationWithNoReturnAsync();
    // ...
}
```

异步方法的异常存储在 `Task` 中，在 `await` 时抛出。无法在 `async void` 方法中捕获异常。
...
using (var cts = new CancellationTokenSource()) {
    ...
    try {
        ...
        Console.WriteLine(ex.Message);   // A task was canceled.
    }
}
```
...
```csharp
async Task ProgressTestAsync(IProgress<int> progress) {
    // 模拟分步请求
    for (int i = 0; i < 20; i++) {
        // 模拟网络请求
...
await ProgressTestAsync(progress);
Console.WriteLine($"Task Completed : {progress}");
```

>---
#### 6.4. 属性

属性可以具有访问器 `get`、`set` / `init`（仅实例，仅调用一次），支持继承，支持 `ref`、`readonly`、`required`。自动属性由编译器生成关联后台字段。

访问器可以具有比属性更强的访问限制。`value` 指代 `set` 传递的参数，`field` 指代默认关联字段。

```csharp
class Person
{
    public string? Name { get; init => field = value?.Trim() ?? ""; } = "";   // 初始值设定
    public int Age { get; private set => field = value > 0 ? value : 0; } 
}
```

只读 `get` 属性可在构造函数中初始化；`init` 属性在初始化器、`with` 表达式、构造函数、或其他 `init` 中初始化。

```csharp
Person p = new() { Name = "Ychao", Age = 10 };  // 初始化器
```
...
`readonly` 属性（仅结构）或 `readonly get` 无法修改实例状态。
...
`required` 属性要求对象初始化时设定值。派生无法取消基类已有 `required` 限定，但可以添加 `required` 限定。
...
索引器可以像数组一样对对象进行索引，支持重载和继承。后台默认生成 `Item` 属性，可由 `IndexerNameAttribute` 更改名称。支持 `ref` 或 `readonly` 限定，`ref` 索引器无 `set` / `init`。
...
类型声明中包含属性 `int Length {get;}` 和索引器 `this[int]`，且无 `this[System.Index]` 和 `this[System.Range]` 时，类型对象隐式支持索引运算；包含方法 `public TResult[] Slice(int start, int length)`，则隐式支持范围运算。
...
事件是 *sender* 能够向所有相关组件广播已发生事情的一种方式。任意组件都可以订阅（`+=`）和取消（`-=`）事件。事件建立在委托基础之上，作为委托类型的包装器。事件由声明方内部触发和初始化，外部只能订阅和取消订阅。字段形式的事件自动合成 `remove` 和 `add` 访问器。
...
委托和事件均通过调用在运行时绑定的方法组来进行组件之间的通信，都支持单播和多播方式。程序设计基于方法回调时常使用委托；程序在不调用任何订阅方并完成其所有工作时常使用事件。

>---
#### 6.7. 重载运算符

类或结构可重载运算符，至少有一个参数是包含类型。可重载的运算符有：
...
记录类型隐式生成复制构造函数。`with` 表达式自动调用它的复制构造函数。可以显式创建复制构造函数：
...
静态构造函数在首次调用类型时仅调用一次；接口静态构造函数在首次使用接口任意静态成员时被调用。静态构造中引发异常时，类型在程序生存期内均将保持未初始化，后续任意调用都会引发 `TypeInitializationException`。
...
析构函数在类中定义，由垃圾回收器调用。析构函数可以在实例符合终结条件后的任何时间、任何线程上执行。当实例终结时按派生顺序调用析构函数。可以调用 `GC.Collect` 手动进行垃圾回收。
...
可以在多个编译单元进行分部声明（类、结构、记录、接口）；分部类型中声明分部成员（事件、实例构造函数、方法、属性、嵌套类型）。对于 `partial void func();` 分部方法可以不提供实现，所有的无效调用被删除。
...
接口可以包含方法、属性、事件、索引器、静态构造函数、静态成员、常量、运算符、嵌套类型等。
...
派生可以隐式或显式接口方式实现接口，泛型接口实现需满足约束；隐式实现可以更改为更高访问性限定，属性和索引器可以添加其他访问器。派生重写方法将更改接口映射关系。
...
接口可以声明静态 `abstract` 和 `virtual` 函数类成员，常在泛型接口中声明，由类型参数静态访问。`sealed` 或 `virtual` 静态成员提供默认实现。
...
记录 `record struct/class` 用于封装数据。编译器为主构造函数的位置参数自动生成公有属性和元组解构函数 `Deconstruct`（可重定义）：
...
方法的约束从基方法继承，无法指定（除了 `class` 或 `struct`）或更改约束。
...
类型参数（引用类型）的协变（`out`）或逆变（`in`）可以使用派生程度比目标构造类型更高（协变）或更低（逆变）的类型实参，仅限于泛型接口和委托类型。逆变参数常用作方法参数类型，协变参数常用作返回类型。接口不会继承基接口的变体特性。
...
实现变体接口时，有时会导致多义性。编译器不会产生错误，未显式指定由 CLR 选择（例如继承顺序先后）。
...
`pt ± n` 相当于 `(T*)((nint)pt ± n*sizeof(T))`。`pt_x - pt_y` 相当于 `((nint)(pt_x) - (nint)(pt_y)) / sizeof(T)`。
...
固定变量不受垃圾收集影响（包括局部变量、值形参和解引用指针创建的变量），取址不受限制；可移动变量可由垃圾收集器重新定位或处理（包括对象字段、数组元素、引用传递的参数），在 `fixed` 固定语句可取址。
...
`delegate*` 语法声明函数指针，对静态方法取址。委托使用 `callvirt` 指令调用虚函数，函数指针使用 `calli` 指令调用。在性能关键的代码路径中，`calli` 调用效率更高。调用约定也是函数指针签名的一部分。
...
>---
#### 15.2. 迭代语句: for, foreach, do, while
...
`await foreach` 支持异步数据流，迭代器返回 `IAsyncEnumerable<T>` 或 `IAsyncEnumerator<T>`。`IAsyncDisposable` 用于异步清理。迭代类型具有 `GetAsyncEnumerator` 方法，返回类型具有 `Current` 属性和 `ValueTask<bool> MoveNextAsync` 方法。
...
>---
#### 15.3. 跳转语句: break, continue, goto, return
...
异常派生自 `Exception` 类型，`e.StackTrace` 保存异常发生时的调用堆栈状态。`e.Message` 保存发生异常信息。`throw` 抛出异常，隐式转换为任意类型；`try` 执行安全检查；`catch` 执行异常捕捉；`when` 执行异常筛查；`finally` 执行资源清理。`catch` 块中 `throw;` 重新引发异常，并保留异常的原始堆栈跟踪在 `e.StackTrace`。
...
特性 `Attribute` 向程序实体声明中添加元数据，运行时通过反射获取这些元数据信息。显式标识可以是：
- `assembly`：程序集。
- `module`：模块。
- `event`：事件。
...
[type: SomeAttr]
class Sample<[typevar: SomeAttr] T> where T : notnull {
    [field: SomeAttr] public T AutoValue { get; } 
    public T Value {
        [return: SomeAttr]
        get => default!;
        [param: SomeAttr]
    ...
    [field: SomeAttr] event Action fieldEvent;
...
运行时通过反射检索类型绑定的特性信息。一个特性由特性类、位置参数和命名参数定义。
...
通过指定 `.locals init` 标志，强制从本地内存池分配的所有内容进行零初始化。`SkipLocalsInit` 抑制此标志。 
...
`Experimental` 特性标记该程序实体是实验性的。使用被标记的目标时，编译器发出诊断错误。该错误可以通过将调用方也标记为 `[Experimental]` 来绕过诊断，或者通过编译器 `#pragma warning disable 'diagnosticId'` 显式允许实验代码通过编译：
...
查询表达式用于查询和转换启用 LINQ 数据源中的数据。可查询类型：
- 内存集合的查询：`IEnumerable<T>` 编译为委托。`IQueryable<T>` 查询。`Enumerable` 提供对 `IEnumerable<T>` 的查询运算符。
- 远程数据源的查询：`IQueryable<T>` 编译为表达式树。`Queryable` 提供对 `IQueryable<T>` 的查询运算符。
- 异步查询：`AsyncEnumerable` 提供对 `IAsyncEnumerable<T>` 的查询运算符。
- 并行查询：`ParallelEnumerable` 提供对 `IEnumerable<T>` 的并行查询运算符。

查询指定要从数据源中检索的信息，并指定如何排序、分组和结构化。查询的实际执行在 `foreach` 中循环访问查询变量之后进行。大部分查询函数在 `foreach` 中延迟执行，查询表达式结果可调用 `ToList` 或 `ToArray` 立即执行。
...
var numQuery = from num in numbers
                ...
                select num;
// 等价 Linq 语法
IEnumerable<int> _numQuery = numbers.Where(num => num % 2 == 0)
                                    .OrderBy(n => n);
// 查询延迟执行
foreach (int num in numQuery)
    Console.Write("{0,1} ", num);
// 查询立即执行 
Console.WriteLine("\n" + string.Join(" ", numQuery.ToArray()));
```
...
`group` 子句返回的 `IGrouping<TKey,TElement>` 键值对是一个由列表组成的列表，需使用嵌套 `foreach`，外部循环访问键，内部循环访问组项。
...
`orderby` 返回升序或降序序列，默认为 `ascending`，降序为 `descending`。*Primary* 执行主要排序。*secondary* 对每个主要排序组的元素执行次要排序。

```csharp
orderby <Primary>[,<secondary>...]? [ascending/descending]?

var query = from word in new string[] { "the", "quick", "brown", "fox", "jumps" }
            orderby word.Length ascending, word[0] descending
            select word;
...
`join` 子句将两个无关序列相关联（同等联接），要求序列中的每个元素能与其他序列进行某种比较。`equals` 比较指定的键是否相等（值相等性）。`join` 输出形式取决于执行的联接类型：内部联接、分组联接、左外部联接。
...
查询表达式可以转换为 LINQ 的查询运算符。`Linq.Enumerable` 适用于 `IEnumerable<T>` 对象，`Linq.Queryable` 适用于 `IQueryable<T>` 对象。查询运算符提供包括筛选、映射、聚合、排序等在内的查询功能。
...
`ParallelEnumerable` 提供对 `IEnumerable<T>` 的并行查询支持。

| Func                      | Destription                                |
| :------------------------ | :----------------------------------------- |
| `AsParallel`              | 转换为 `ParallelQuery<T>` 以启用并行查询   |
| `AsOrdered`               | 保持并行查询的原始元素顺序                 |
| `AsSequential`            | 并行查询返回顺序查询                       |
| `WithDegreeOfParallelism` | 设置最大并行度                             |
| `WithCancellationToken`   | 设置取消令牌                               |
| `WithExecutionMode`       | 设置执行策略                               |
| `WithMergeOptions`        | 设置并行结果的合并选项                     |
| `ForAll`                  | 并行对每个元素执行操作                     |

>---
#### 17.4. 表达式树

表达式树（`System.Linq.Expression`） 以树结构表示表达式。

```csharp
Expression<Func<int, int, int>> expr = (int x, int y) => x + y;
expr.Compile().Invoke(10010, 10086);
```

> 表达式树节点

```csharp
Expression<Func<bool, string?>> exprTree = (bool cond) => cond ? "Hello World" : null;
// 解释表达式
ParameterExpression param = exprTree.Parameters[0];
ConditionalExpression condExpr = (ConditionalExpression)exprTree.Body;
ParameterExpression cond = (ParameterExpression)condExpr.Test;
ConstantExpression ifTrue = (ConstantExpression)condExpr.IfTrue;
ConstantExpression ifFalse = (ConstantExpression)condExpr.IfFalse;
Console.WriteLine($"Decomposed expression: ({param.Type} {param.Name}) => {cond} ? {ifTrue} : {ifFalse}" +  $"\nReturn Type: {exprTree.ReturnType}");
/*
Decomposed expression: (System.Boolean cond) => cond ? "Hello World" : null
...
var mul = Expression.Parameter(typeof(int), "mul");
var exit = Expression.Label(typeof(int));
var block = Expression.Block(
    // int mul = 1
    [mul], Expression.Assign(mul, Expression.Constant(1)),
    // while(true) {
    Expression.Loop(
        // if(n > 1)
        Expression.IfThenElse(Expression.GreaterThan(n, Expression.Constant(1)),
            // mul *= n--
            Expression.MultiplyAssignChecked(mul, Expression.PostDecrementAssign(n)),
        // else return
        Expression.Return(exit, mul)),
    // } // end loop
    exit)
    );
var factorial = Expression.Lambda<Func<int, int>>(block, n);

Console.WriteLine(factorial.Compile().Invoke(5));   // 120
```
