...
C# 定义了 8 种变量类别：静态变量、实例变量、数组元素、值参数、输入参数、引用参数、输出参数和局部变量。`default` 表达式用于零初始化。`ref` 创建变量引用。支持弃元 `_`。
...
`var` 隐式声明局部变量，编译器类型推断。`var` 作为 `out` 参数传递，发生歧义需要指定类型。
...
`implicit` 和 `explicit` 用于定义隐式转换或显式转换运算。无法声明接口之间的用户定义转换。如果两个类型之间存在预定义的转换，则忽略这些类型之间的任何用户定义转换。
...
类型参数（引用类型）的协变（`out`）或逆变（`in`）可以使用派生程度比目标构造类型更高（协变）或更低（逆变）的类型实参，仅限于泛型接口和委托类型。逆变参数常用作方法参数类型，协变参数常用作返回类型。
