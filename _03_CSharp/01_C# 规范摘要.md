...
| token                                                                                                                                                                                                                    | description        |
| :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----------------- |
| `void`,`bool`,`char`,`string`,`object`,`dynamic`<br>`sbyte`,`short`,`int`,`long`,`byte`,`ushort`,`uint`,`ulong`<br>`nint`,`nuint`,`float`,`double`,`decimal`,<br>`enum`,`struct`,`class`,`record`,`interface`,`delegate` | 类型声明关键字     |
| `null`,`true`,`false`                                                                                                                                                                                                    | 文本关键字         |
...
| `abstract`,`override`,`virtual`,`sealed`,`event`,`record`,`async`,<br>`const`,`volatile`,`readonly`,`event`,`extern`,`unsafe`,<br>`in`(逆变),`new`(成员),`out`(协变)                                                     | 类型修饰符         |
| `ref`,`out`,`ref readonly`,`in`,`params`                                                                                                                                                                                 | 方法参数修饰符     |
| `base`,`this`                                                                                                                                                                                                            | 访问关键字         |
| `add`,`remove`,`get`,`set`,`value`,`field`,`init`,<br>`partial`,`scoped`,`require`,`when`,`extension`                                                                                                                    | 上下文关键字       |
| `operator`,`explicit`,`implicit`                                                                                                                                                                                         | 运算符             |
...
C# 是强类型语言，包含值类型和引用类型。值类型在堆栈（*stack*）上分配，引用类型在堆（*heap*）上分配。值类型包括简单类型、枚举、结构、元组和可空值等；引用类型包括类、接口、数组和委托等。托管对象在托管堆上分配内存，变量只保留对象引用。CLR 自动执行内存管理和垃圾回收。
...
C# 定义了 8 种变量：静态变量、实例变量、数组元素、值参数、输入参数、引用参数、输出参数和局部变量。`default` 表达式用于零初始化变量。支持弃元 `_`。
...
```csharp
ref int REF(ref int rv)    // ref paramter
{
    ref var _rv = ref rv;  // ref local
    return ref rv;         // ref return 
}
```
...
类成员可以是常量、字段、方法、属性、索引器、运算符、事件、终结器、实例或静态构造函数、嵌套类型。
