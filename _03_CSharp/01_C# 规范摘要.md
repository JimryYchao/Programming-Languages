...
`Main` 可选包含 `string[] args`。程序名称不是 `args` 首元，但是 `Environment.GetCommandLineArgs()` 首元。
...
在单独一个编译单元以顶级语句替代 `Main`，编译器为其自动生成正确的 `Main`。
...
C# 是强类型语言，包含值类型（*stack*）和引用类型（*heap*）。值类型包括简单类型、枚举、结构、元组和可空值类型等；引用类型包括字符串、类、接口、数组和委托类型等。托管（*managed*）对象在托管堆上分配内存，变量只保留对象引用。CLR 自动执行内存管理和垃圾回收。
...
所有类型（除指针和 `ref struct` 外）的最终基类是 `System.Object`，这样的统一类型层次结构称为通用类型系统（CTS）。*装箱和拆箱* 提供值类型和引用类型之间互相转换的桥梁。
...
命名空间全局可见，类型和成员具有访问性限制，继承成员的访问性从基类继承。`new` 声明隐藏同名继承成员。
...
class Sample {
    ...
    {
        int i = 10;  // 局部变量
        scoped ref int ri = ref c;   // 局部变量引用
        // ...
    }
}
```
...
命名空间 `namespace` 对程序元素进行逻辑分组。每个元素在其名称空间单一定义。`global::` 表示全局命名空间。
...
`using` 引入命名空间成员；`using static` 导入类型成员；`using Id = T` 创建别名；`global using` 表示全局指令。
...
在 `struct`、`class`、`record`、`interface` 中声明解构函数 `Deconstruct` 或扩展方法，拥有至少两个 `out` 参数，可重载。主构造函数记录自动生成 `Deconstruct` 方法。
...
`$` 内插字符串中的内插表达式（`{expr}`）替换为表达式结果（`ToString`）的字符串形式。`{{` 和 `}}` 转义为 `"{"` 和 `"}"`。
...
结构可以实现接口；数据成员具有初始值时，需要显式提供实例构造函数。
...
`ref struct` 在堆栈（*stack*）上分配：
  - 可以实现接口，或类型参数（`allows ref struct`）。
  - 不能是 `class` 或非 `ref struct` 成员、数组或元组元素。
...
>---
#### 17.2. 迭代语句: for, foreach, do, while
...
>---
#### 17.3. 跳转语句: break, continue, goto, return
...
特性类具有位置参数和命名参数。命名参数由特性类的非静态可读写的字段或属性定义。参数类型仅限于：`bool`、`char`、`byte`、`sbyte`、`short`、`ushort`、`int`、`uint`、`long`、`ulong`、`float`、`double`、`object`、`System.Type`、枚举类型以及对应的一维数组。`Type` 的特性参数可以是未绑定的泛型类型。
