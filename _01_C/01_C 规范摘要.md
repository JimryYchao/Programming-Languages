...
C 程序以源文件为单元保存，源文件及导入头文件称为预处理翻译单元。翻译单元可单独编译后通过链接生成可执行程序，单元间通过外部链接的函数/对象或数据文件通信。

翻译阶段：
1. **字符映射**：物理源文件多字节字符映射到内部源表示形式。
2. **行拼接**：删除反斜杠 `\`，拼接物理源行以形成逻辑源行。
3. **词汇切分**：词汇分析时分为 *预处理令牌分析* 和 *空白字符替换注释*。
4. **预处理**：执行预处理指令，扩展宏，递归处理 `#include` 文件并删除所有预处理指令。
5. **字符集映射**：字符常量和字符串文本转换为执行字符集。
6. **字符串串联**：相邻字符串和宽字符串文本串联。
7. **翻译**：删除空格，预处理令牌转换为 *token*。
8. **链接**：解析外部引用，链接库组件，并生成可执行程序。

>---
#### 9.2. 执行环境

C 定义两种执行环境：独立式和托管式。程序启动时调用`main`函数，静态存储对象在启动前初始化。
...
*编译器* 将源代码转换为中间代码，支持模块化开发，或仅需编译发生更改的模块。*链接器* 将目标文件和系统组件打包成可执行文件。
...
流（stream）是数据序列的抽象，包括文本流和二进制流。文本流是由行组成的有序字符序列；二进制流是一个有序的字符序列。每个流都有一个方向，调用 `freopen` 或 `fwide` 函数能改变流的方向。

每个面向宽的流都有一个关联的 `mbstate_t` 对象，用于存储流的当前解析状态。每个流都有一个相关的锁，用于防止多个执行线程访问同一流时的数据竞争，并限制多个线程执行的流操作的交错。所有读、写、定位或查询流位置的函数在访问流之前都会锁定流。当访问完成时，与流关联的锁被释放。

> Files（文件）

通过打开文件，流与外部文件（可能是物理设备）相关联。当流 **无缓冲** 时，字符将从源端或目标端尽快出现，字符可能会作为一个块被累积和传输到主机环境或从主机环境传输出去；当流 **全缓冲** 时，在缓冲区被填满时，字符将作为一个块传输到主机环境或从主机环境传输出去。当流 **行缓冲** 时，在遇到换行符时，字符将作为块传输到主机环境或从主机环境传输出去。在缓冲区被填满时、或者是在未缓冲的流上请求输入时、或者在需要从主机环境传输字符的行缓冲流上请求输入时，字符将作为块传输到主机环境中。

可以通过关闭文件来解除文件与控制流的关联。输出流在与文件解除关联之前被刷新，任何未写入的缓冲区内容都被传输到主机环境。`FILE` 对象的生存期在相关文件（包括标准文本流）关闭时结束。该文件可能随后由相同或另一个程序执行重新打开，并回收或修改其内容。程序终止之前正确关闭所有打开的文件。其他终止程序（`abort`）不需要正确关闭所有文件。

在程序启动时，三个文本流是预定义：标准输入（`stdin`）、标准输出（`stdout`）和标准错误（`stderr`）。在最初打开时，标准错误流未被完全缓冲；当且仅当标准输入和标准输出流可以被确定为不引用交互式设备时，该流才被完全缓冲。

宽字符输入函数从流中读取多字节字符，并将其转换为宽字符，就像连续调用 `fgetwc`。每次转换都像调用 `mbrtowc`，转换状态由流自身的 `mbstate_t` 对象描述。字节输入函数从流中读取字符，就像连续调用 `fgetc`。

宽字符输出函数将宽字符转换为多字节字符，并将其写入流，就像连续调用 `fputwc`。每次转换都像调用 `wcrtomb`，转换状态由流自身的 `mbstate_t` 对象描述。字节输出函数将字符写入流，就像连续调用 `fputc`。
