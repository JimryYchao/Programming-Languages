...
extern void func();    // 外部链接，其他文件中定义
double PI = 3.14159265; // 全局变量，外部链接
int main(void)  /* 函数范围 */
{
	/* 块范围 */
...
**字符映射** > **行拼接** > **词汇切分** > **预处理** > **字符集映射** > **字符串串联** > **翻译** > **链接**	

> 执行环境

C 定义了两种执行环境：独立环境或托管环境。程序启动时调用入口（托管环境为 `main`）函数。
...
```powershell
# 预处理：插入头文件，删除注释，处理预处理：插入头文件，删除注释，处理预处理指令。
...
| :---------- | :------------------------------------------------------------------------------------------------------------ |
| 真值        | `nullptr`,`false`,`true`                                                                                      |
| 说明符      | `extern`,`static`,`_Atomic`,`register`,`thread_local`,`static`,`restrict`,`inline`,`const`,`volatile`         |
| 类型声明    | `signed`,`unsigned`,`auto`,`void`,`bool`,`char`,`short`,`int`,`long`,`float`,`double`,`enum`,`struct`,`union` |
| 语句关键字  | `if`,`else`,`switch`,`case`,`default`,`do`,`while`,`for`,`break`,`continue`,`return`,`goto`                   |
| 表达式语句  | `sizeof`,`constexpr`,`typeof`,`typeof_unqual`,`static_assert`,`typedef`,`_Generic`                            |
| 类型对齐    | `alignas`,`alignof`                                                                                           |
| 扩展类型    | `_Imaginary`,`_Complex`,`_BitInt`,`_Decimal32`,`_Decimal64`,`_Decimal128`                                     |
...
| 标准浮点数   | `float`,`double`,`long double`                    |
...
```c
enum Month : unsigned char {  // default : int
...
结构或联合可包含位字段，类型是 `bool`、`int`、`char`、`short`，位字段不能越界存储。匿名位字段表示填充，0 强制整数边界对齐。
...
函数域内声明或 `typedef` 定义可变长度数组（VLA），VLA 无法限定为 `thread_local`、`static`、`extern`。
...
灵活数组成员仅作为结构的最后一个成员，动态分配其空间。`sizeof` 不计算灵活数组成员。
...
C 字符串是零终止符结尾的字符数组，编译器优化会将相同字符串值的保存为单个副本。

```c
char* s = "Hello";		   // C style string
char* hi = "Hello" "World" // 字符串拼接
...
```c
static auto Hi = "Hello World";   // char*
...
文件范围 `static` 声明内部链接、静态局部变量。`extern` 声明外部链接或引用。
...
`_Atomic` 限定原子类型（非数组、函数、其他原子类型或限定类型）。
...
原子类型对象是仅有的免除数据竞争的对象。对于所有原子运算，保证有四种连贯：写写连贯、读读连贯、读写连贯、写读连贯。
...
| 逗号运算     | `(expr1, expr2, ... exprN)`  返回最后一个表达式结果                                                   |
...
函数原型仅充当函数声明，被调用函数必须具有定义。不能返回数组或函数类型。数组参数实际上为指针。
...
double sum(int lim, ...) /* 设 lim 表明变参列表中参数的数量 */
{
	...
	double f1 = va_arg(ap, double); // ap[0], va_arg(ap, type)
	va_copy(ap_copy, ap); // copy ap[1:] to ap_copy
	va_end(ap);  // clear ap
...
在仿函数宏中，`#` 对 *替换列表* 字符串化并做形参替换（`#X` 替换为 `"X"`）。
...
流是数据序列的抽象，分为文本流和二进制流。流具有方向，通过 `freopen` 或 `fwide` 改变。宽流关联 `mbstate_t` 对象存储流的状态。预定义文本流：`stdin`、`stdout`、`stderr`。

调用 `fopen` 打开外部文件（或物理设备）并返回一个关联流指针。流分为 **无缓冲**、**全缓冲**、**行缓冲** 三种缓冲模式。`close` 关闭文件并解除流关联。
