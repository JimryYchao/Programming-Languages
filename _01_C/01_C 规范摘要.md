...
范围（作用域）分为全局范围、文件范围、函数范围、块范围。存储期（生存期）分为静态存储、线程存储、自动存储或动态存储期。链接包括外部链接（`extern`）、内部链接（`static`）或无链接。
...
C 源文件及导入头文件称为预处理翻译单元。翻译单元可单独编译后通过链接生成可执行程序，翻译单元之间通过外部链接的函数、对象或数据文件进行通信。源文件翻译阶段：
...
C 定义了两种执行环境：独立环境或托管环境。程序启动时调用入口（托管环境为 `main`）函数，静态存储对象在启动前初始化。

- **程序启动**：调用入口函数 `void/int main` 函数。
- **程序执行**：托管环境可使用标准库提供的所有功能。
- **程序终止**：从 `main` 返回等同于调用 `exit`。
...
C 编译器将源代码转换为中间代码，或仅编译更改的模块。链接器将目标文件和系统组件打包成可执行文件。

- Windows 编译过程：源代码（\*.c） ➡ 编译器 ➡ 目标代码（\*.obj） ➡ 链接器（链接启动代码和库代码） ➡ 可执行文件（*.exe）

+ Unix 编译过程：输入源代码 ➡ 文本编辑器 ➡ 源代码（\*.c） ➡ 编译器 ➡ 可执行代码（\*.out） 

编译分为四阶段：

```powershell
# 1. 预处理：插入头文件，删除注释，处理预处理指令。
   gcc -E main.c -o hello.i
# 2. 编译：将预处理文件翻译成汇编文件。
   gcc -S main.c -o hello.s
# 3. 汇编：将汇编文件转成二进制目标代码。
   gcc -c main.c -o hello.o
# 4. 链接：合并目标文件和库文件，生成可执行程序。
   gcc main.c -o a.out
```
...
- 结构体对齐规则：首成员位于结构偏移量 0 地址。其他成员对齐为 **min { 自身对齐，对齐系数 }** 整数倍的地址。总体对齐时，结构体字节大小为 **min { 成员最大自身对齐，对齐系数 }** 的整数倍。
- 对齐系数：宏 `#pragma pack(N)` 设置对齐系数，N 是 2 的幂次方，1、2、4 等。0 表示本机默认对齐。`#pragma unpack()` 取消对齐。
...
结构或联合可包含位域，位字段的基础类型是 `bool`、`int`、`char`、`short`，位字段的存储不能越界。匿名位字段表示填充，零值匿名位字段表示下一个位字段与下一个整数边界对齐。
...
可以为任何基本类型、结构、联合、函数、数组和其他指针对象创建关联指针（除位域、寄存器变量）。`void*` 表示无类型指针。数组的标识符被视为指向数组首元的指针（`arr = &arr[0]`，`arr[n] = *(arr+n)`，`&arr[n+1] = &arr[n]+1`）。
...
C 数组具有相同类型、连续存储数组元素的定长对象序列，无零长数组。试探性声明 `int arr[]` 无定义关联时被视为 `arr[] = {0}`。
...
可以在函数域内声明或 `typedef` 定义可变长度数组（VLA），这类对象称为可变修改对象（VM）。VLA 无法限定为 `thread_local`、`static`、`extern`。VLA 指针声明可以是 `static`。
...
灵活数组成员可作为结构的最后一个成员，由分配函数分配灵活数组的空间。
...
C 字符串是具有 `char` (`wchar_t`) 的类型数组，`'\0'` (`L'\0'`) 自动追加到字符串末尾。编译器优化策略会将具有相同字符串值的单个副本置于可执行文件中。

```c
char *str1 = "Hello";
int main(void) {
	char *str2 = "Hello";
	char str3[] = "Hello";    // 字符串初始化字符数组
	printf("pStr1: %p\n", str1);
	printf("pStr2: %p\n", str2);
	printf("pStr3: %p\n", str3);
...
`constexpr` 声明常量表达式，编译时计算，文件范围内默认具有内部链接。
...
#### 3.1. 链接：static, extern

文件范围 `static` 声明内部链接，块范围限定局部变量为静态存储。`extern` 声明外部链接或外部引用（默认）。文件范围变量默认具有静态存储。
...
`thread_local` 声明线程存储的 `static` 或 `extern` 线程对象，每个线程具有其独立副本。
...
原子类型的对象是仅有的免除数据竞争的对象。每个原子对象都拥有关联自身的修改顺序。对于所有原子运算，保证有四种连贯：写写连贯、读读连贯、读写连贯、写读连贯。原子属性仅对左值表达式有意义。左值到右值转换会把原子性及其他限定符剥离。
...
`alignas/_Alignas` 为对象类型请求更为严格的对齐，`alignof/_Alignof` 返回完整对象类型的对齐要求。**基本对齐** 是小于等于 `alignof(max_align_t)` 的有效对齐，**扩展对齐** 是大于 `alignof(max_align_t)` 的对齐表示。
...
`alignas` 不在 `typedef`、位域、函数、函数参数、`register` 声明中使用，零对齐没有任何作用。`struct` 和 `union` 对齐方式与其成员的最大对齐方式相等。
...
类型转换包含隐式或强制转换（`(type)value`）。任何标量转换为 `bool` 时，数值零、空指针或 `nullptr` 结果为 `false`。指针之间仅支持强制转换，没有函数指针和对象指针（含 `void *` ）间的转换。
...
函数原型充当无定义的函数声明，被调用函数必须具有定义。函数不能返回数组或函数类型。
...
属性为各种源构造（如类型、对象、标识符或块）指定其他信息，不受支持的属性被忽略。
...
`[[noreturn]]` 指示函数不会由于执行返回或抵达函数体结尾而返回（一般通过执行 `longjmp` 返回）。
...
|:--|:--|
|空指令|`#`|
| 条件编译|  `#if`,`#else`,`#elif`,`#endif`,<br>`#ifdef`,`#ifndef`,`#elifdef`,`#elifndef` | 	
...
条件编译根据特定条件选择性地编译代码块。条件表达式可以具有形式 `defined` 的一元运算符：
...
为了避免预处理头文件发生无限递归，通常会使用头文件防护（或 `#pragma once`）：
...
条件表达式搜索 `__has_include` 头文件或源文件，搜索成功返回 1。
...
`#embed` 嵌入一个二进制资源，可作为给定嵌入参数的二进制数据序列。嵌入的资源具有实现的资源宽度，或由嵌入参数 `limit` 修改。嵌入元素宽度等于 `CHAR_BIT`，或由某个由实现定义的参数所修改。资源宽度必须可被嵌入元素宽度整除，即 *资源宽度* % *嵌入元素宽度* = 0。
...
`#embed` 旨在将二进制资源数据转换为一系列整数常量表达式，以尽可能保留资源位流的值。
...
嵌入参数 `suffix` 最多出现一次，它必须带有（可能为空的）实参子句。如果资源非空，则该参数子句的内容被置于紧跟该指令的展开之后。否则，它没有效果。
...
嵌入参数 `prefix` 最多出现一次，它必须带有（可能为空的）实参子句。如果资源非空，则该参数子句的内容被置于紧接该指令的展开之前。否则，它没有效果。
...
嵌入参数 `if_empty` 最多出现一次。它必须带有（可能为空的）实参子句。如果资源为空，则以该参数子句的内容替换该指令。否则，它没有效果。
...
- `__STDC_EMBED_NOT_FOUND__  0`：搜索资源失败；或者嵌入参数不受 `#embed` 支持；
- `__STDC_EMBED_FOUND__  1`：搜索资源成功，且嵌入参数受支持，资源不为空；
- `__STDC_EMBED_EMPTY__  2`：搜索资源成功，且嵌入参数受支持，但资源为空。
...
#### 8.6. 诊断：error, warning

`#error` 和 `#warning` 产生一个诊断消息。`#error` 终止编译，`#warning` 仅发出警告。
...
`#pragma` 控制编译器行为，如禁用编译器警告或更改对齐要求等。
...
- `FENV_ACCESS OFF` 默认是关闭，表示编译器程序访问或修改浮点环境时，禁用可能推翻标志测试和模式更改（例如，全局共用子表达式删除、代码移动，及常量折叠）的优化。
- `FP_CONTRACT ON` 允许缩略浮点表达式，即忽略舍入错误和浮点异常的优化。
- `CX_LIMITED_RANGE OFF` 告知编译器复数乘法、除法，及绝对值可以用简化的数学公式，不考虑中间溢出的可能性。
- `FENV_ROUND` 浮点舍入方式。
...
流是数据序列的抽象，分为文本流（由行组成的字符序列）和二进制流（原始字符序列）。每个流有方向，可通过 `freopen` 或 `fwide` 改变。每个面向宽的流都有一个关联的 `mbstate_t` 对象存储流的当前解析状态。每个流都有一个相关的锁，防止多个执行线程数据竞争。

在程序启动时，三个文本流是预定义：标准输入（`stdin`）、标准输出（`stdout`）和标准错误（`stderr`）。

> Files（文件）

通过打开文件，流与外部文件（或物理设备）相关联。当流 **无缓冲** 时，字符将从源端或目标端立即传输；当流 **全缓冲** 时，在缓冲区填满时传输；当流 **行缓冲** 时，遇到换行符传输。可以通过关闭文件来解除文件与流的关联。
...
/* format 格式说明符
    ...
    * 占位符    表示精度或宽度
*/
printf("%*d", -5, 10);   // - 左对齐, 字符宽度至少为 5, 10 对应 "%d"
