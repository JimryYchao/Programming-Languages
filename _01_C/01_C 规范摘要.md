...
数组变量被视为指向数组首元的指针。不允许声明零长数组，试探性声明暂时被视为具有一个数值为 0 的数组。
...
灵活数组成员作为结构的最后一个成员，且至少有一个非灵活数组成员，由分配函数分配数组所需的额外空间。`sizeof` 灵活数组成员。灵活数组结构只能作为其他结构的最后一个成员（保证数组成员为结构布局的最后一个成员），无法创建灵活数组结构的数组。
...
结构可包含比整型类型占用更少存储空间的位域成员。位字段只能是 `bool` 或 `int`、`char`、`short`，位字段宽度不超过其基础整数长度，不能跨越整数边界和宽度限定范围。
...
联合所有成员共享单个存储，与最大成员字节相同。联合可以具有位域，但不包含不完整类型、`void` 或函数类型。
...
可以为任何基本类型、结构或联合类型、函数、数组和其他指针对象创建关联指针（除了位域、寄存器变量）。`void*` 表示无类型指针，可以强制转换为（除 `char*` 类型外，隐式）其他类型的指针。指针可以是 CV 限定。
...
int main(void)
{
    ...
    auto v1 = *ptr1;    // 100
    // 指针与整数加减，递增递减
    ptr1 = ptr1 + 1;    // 相当于 ptr1++, arr[1]
    // 指针相减
    ptrdiff_t off = ptr2 - ptr1;    // 2
}
```
...
字符串是以 `\0` 结尾的字符序列。UTF8 字符串以 `u8` 为前缀，`wchar_t` 以 `L` 为前缀，UTF16 以 `u` 为前缀，UTF32 以 `U` 为前缀。字符串拼接要求它们的前缀相同或没有前缀。
...
`constexpr` 声明常量表达式；限定类型不能是原子类型、VM 类型、`volatile` 或 `restrict` 限定。文件范围 `constexpr` 表达式默认具有内部链接。
...
>---
#### 3.6. thread_local 线程存储声明

`thread_local` 声明线程存储期、`static` 或 `extern` 限定的非自动变量。线程对象在启动线程时开始初始化，每个线程拥有其唯一副本。
...
文件范围 `restrict` 指针对访问动态分配的全局数组很有用。
...
函数原型充当无定义的函数声明，被调用的函数必须具有定义。返回类型为 `void` 、除数组和函数类型之外的任意完整对象类型。
...
double sum(int lim, ...) /* 设 lim 表明变参列表中参数的数量 */
{
	// 1. 声明一个储存参数的对象
	va_list ap;
	// 2. 把 ap 初始化为参数列表, lim 为可变参数前置参数
	va_start(ap, lim);
	// 3. 访问参数列表的内容 va_arg(ap, 类型名)
	//---- 检索第 1 个参数
	double tic = va_arg(ap, double);   // 传入的参数类型要求与宏参数的类型相匹配 
	//---- 检索第 2 个参数
	int toc = va_arg(ap, int);
	// 4. 宏完成清理工作
	va_end(ap); 	// 清理工作，释放动态分配用于储存参数的内存 
}
```
