...
- **存储期**：静态存储 `static`、线程存储 `thread_local`、自动存储（局部非静态变量）、动态存储（`malloc`、`calloc`、`realloc`、`free`）。
...
#### 3.4. 存储类：register, thread\_local
...
#### 7.3. unsequenced 和 reproducible 函数类型标准属性
