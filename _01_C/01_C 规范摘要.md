...
作用域（范围）层次结构分为 *全局存储期* > *文件范围* > *函数范围* > *块范围*。所有 C 函数都是全局存储期。变量的 **存储期** 分为 *静态存储期*、*线程存储期*、*自动存储期*、*动态分配存储期*。

标识符通过 **链接** 描述可见性，包括 `extern` 外部链接（默认）、`static` 内部链接、无链接。
...
atomic             // 原子存储，_Atomic                          
register           // 寄存器存储                    
thread_local       // 线程本地存储，_Thread_local 
restrict           // 指针约束声明                      
inline             // 内联函数声明  
signed             // 有符号整数限定                        
unsigned           // 无符号整数限定                        
...
static_assert      // 静态断言，_Static_assert            
_Generic           // 泛型宏                                
```
...
| Suffix     | Description                             |
| :--------- | :-------------------------------------- |
| `f`, `F`   | `float`，单精度 IEEE-754 浮点类型       |
| `l`, `L`   | `long double`，双精度 IEEE-754 浮点类型 |
...
数组变量被视为指向数组首元的指针。无零长数组，试探性声明视为具有一个数值为 0 的数组。
...
可以在函数域内 `typedef` 定义可变长度数组（VLA），这类对象称为可变修改对象（VM）。VLA 无法限定为 `thread_local`、`static`、`extern`。VLA 类型不能是结构或联合的成员。指向 VLA 数组的指针声明可以是 `static`。
...
{
    typedef int VLA[m][m];      // block scope typedef VLA
    int D[m];                   // auto VLA
...
灵活数组成员作为结构的最后一个成员，且至少有一个非灵活数组成员，由分配函数分配灵活数组所需的额外空间。`sizeof` 灵活数组成员。灵活数组结构只能作为其他结构的最后一个成员（保证数组成员为结构布局的最后一个成员），无法创建灵活数组结构的数组。
...
结构可包含比整型类型占用更少存储空间的位域成员。位字段只能是 `bool` 或 `int`、`char`、`short`，位字段不能跨越整数边界和宽度限定范围。匿名位字段表示填充，0 值匿名位字段强制下一个位字段与下一个整数边界对齐。

```c
struct{
	unsigned int first : 4;   // 表明该字段占用的位数
...
联合与结构相似，但所有成员共享单个存储，与最大成员字节相同。联合可以具有位域。
...
可以为任何基本类型、结构或联合类型、函数、数组和其他指针对象创建关联指针（除位域、寄存器变量）。`void*` 表示无类型指针，可以强制转换为（隐式转换 `char*`）其他类型的指针。在函数中传递结构、联合或数组的指针比传递对象本身效率更高。
...
数组标识符被视为指向数组首元的指针，`&arr[n+1] = &arr[n]+1`。`arr[n]` 相当于 `*(arr + n)` （其中 `arr = &arr[0]`）。
...
窄（宽）字符串实际是具有 `char`（`wchar_t`）的类型数组。`'\0'` 自动追加到字符串末尾。编译器优化策略会将具有相同字符串值的单个副本置于可执行文件中。
...
可以使用字符串文本（或宽字符串文本）初始化字符（或宽字符）的数组。指定数组长度小于字符串长度，则忽略多余字符。
...
>---
#### 3.4. 存储类声明：register, thread_local

`register` 声明寄存器局部变量，实现不支持时忽略。无法获取寄存器变量的地址。
...
`thread_local` 声明线程存储的 `static` 或 `extern` 线程对象，在线程启动时初始化，每个线程拥有其唯一副本。
...
`volatile` 易变限定，对象可能由外部程序合法修改。`volatile` 禁用编译器优化策略。`volatile` 结构或联合时，成员也为易变限定；`volatile` 数组时仅限定元素。
...
原子类型的对象是仅有的免除数据竞争的对象，即它们可以被两个线程共时修改，或先被一个修改再被另一个读取。每个原子对象都拥有关联于其自身的修改顺序。
...
`sizeof` 计算完整类型（非函数）或对象的字节大小，一般为常量表达式（除 VLA）。

```c
size_t szo = sizeof(Obj);
size_t szo = sizeof Obj; 
...
`typeof` 保留原类型的所有限定，`typeof_unqual` 返回原始类型。*typeof* 不应用于位域成员。
...
完整对象类型具有 **对齐要求**。`alignas/_Alignas(expr | type)` 为对象类型请求更为严格的对齐，`alignof/_Alignof(expr | type)` 以字节返回完整对象类型的对齐要求。**基本对齐** 是小于等于 `alignof(max_align_t)` 的有效对齐，**扩展对齐** 是大于 `alignof(max_align_t)` 的对齐表示。

类型 `bool`、`char`、`signed char` 和 `unsigned char` 具有最弱的对齐要求（alignof = 1）。以下类型的对齐要求应为基本对齐：
- 所有原子、限定或非限定的基本类型、枚举类型、指针类型；
- 其元素类型具有基本对齐要求的所有数组类型；
- 指定为完整对象类型的所有类型；
- 仅包含具有基本对齐要求的类型成员的所有结构或联合类型。

```c
#include <stdalign.h>
alignas(SIZE) <type> <identifier>;   // SIZE = 0,1,2,4,8,16,...
size_t sz = alignof(type);
```

`alignas` 不在 `typedef`、位域、函数、函数参数、`register` 声明中使用。指定值为 0 或 2^n（n = 0,1,2,4,8,16,...）的有效对齐，零对齐规范没有任何作用。`alignas` 设置的对齐不小于目标类型的基本对齐。

`struct` 和 `union` 对齐方式与其成员的最大对齐方式相等。当声明中出现多个对齐说明符时，有效对齐是最严格的指定对齐。

```c
alignas(0) alignas(4) alignas(16) struct Sample s;   // 16
```
...
函数原型充当无定义的函数声明，被调用函数必须具有定义。返回类型为 `void` 、除数组和函数类型之外的任意完整对象类型。
...
double sum(int lim, ...) /* 设 lim 表明变参列表中参数的数量 */
{
	...
	double tic = va_arg(ap, double);  // ap[0]  
	int toc = va_arg(ap, int);		  // ap[1]
	// 清理工作
	va_end(ap); 
}
```
...
属性为各种源构造（如类型、对象、标识符或块）指定其他信息。任何不支持的属性标记都将被忽略。
...
属性列表中的显示顺序不重要。实现不应定义没有属性前缀的属性，除非它是标准属性。
...
>---
#### 8.2. 条件编译：if, elif, ifdef, ifndef, elifdef, elifndef, endif
