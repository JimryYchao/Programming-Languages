...
static void example_basic_time(void)
{
	...
	printf("Local time: %s", time_buf);
	// 转换为 UTC 时间
	struct tm* utc = gmtime(&now);
	asctime_s(time_buf, sizeof(time_buf), utc);
	printf("UTC time: %s", time_buf);
	// 计算时间差
	struct tm target = {
		.tm_year = 130, // 2030 - 1900
		.tm_mon = 1,   // December
		.tm_mday = 1
	};
	time_t new_year = mktime(&target);
	double diff = difftime(new_year, now);
	printf("Seconds until 2030/1/1: %.0f\n", diff);
}
...
static void example_custom_format(void)
{
	...
	puts(time_buf);
	// ISO 8601 格式
	strftime(time_buf, sizeof(time_buf), "ISO 8601: %F %T %z", tm);
	puts(time_buf);
}
...
/*
[Basic Time Operations]
Epoch seconds: 1768132799
Local time: Sun Jan 11 19:59:59 2026
UTC time: Sun Jan 11 11:59:59 2026
Seconds until 2030/1/1: 127972801

[Custom Time Formatting]
now: 2026-01-11 19:59:59
ISO 8601: 2026-01-11 19:59:59 +0800

[Measure code execution time]
Loop took 1748500 ns (x=2.72)
CPU time used: 1008.000 ms
*/