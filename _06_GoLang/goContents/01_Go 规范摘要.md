...
- [5. 常量与变量](#5-常量与变量)
- [6. 类型](#6-类型)
	- [6.1. 基本内置类型](#61-基本内置类型)
	- [6.2. String](#62-string)
	- [6.3. Array](#63-array)
	- [6.4. Slice](#64-slice)
	- [6.5. Struct](#65-struct)
	- [6.6. Function](#66-function)
	- [6.7. Interface](#67-interface)
	- [6.8. Map](#68-map)
	- [6.9. Channel](#69-channel)
	- [6.10. Pointer](#610-pointer)
- [7. 命名类型](#7-命名类型)
	- [7.1. 别名声明](#71-别名声明)
	- [7.2. 类型定义](#72-类型定义)
- [8. 类型和值的属性](#8-类型和值的属性)
...
- [11. 方法与接收器](#11-方法与接收器)
	- [11.1. 方法声明](#111-方法声明)
	- [11.2. 方法集](#112-方法集)
	- [11.3. 方法表达式](#113-方法表达式)
	- [11.4. 方法值](#114-方法值)
- [12. 泛型声明](#12-泛型声明)
	- [12.1. 类型参数声明](#121-类型参数声明)
	- [12.2. 类型约束](#122-类型约束)
	- [12.3. 满足类型约束](#123-满足类型约束)
	- [12.4. 类型参数实例化](#124-类型参数实例化)
- [13. 内置函数](#13-内置函数)
	- [13.1. append：切片扩展](#131-append切片扩展)
	- [13.2. copy：切片复制](#132-copy切片复制)
	- [13.3. clear：清零或删除元素](#133-clear清零或删除元素)
	- [13.4. close：关闭通道](#134-close关闭通道)
	- [13.5. complex、real、imag：复数操作](#135-complexrealimag复数操作)
	- [13.6. delete：删除映射元素](#136-delete删除映射元素)
	- [13.7. len、cap：长度与容量](#137-lencap长度与容量)
	- [13.8. make：分配](#138-make分配)
	- [13.9. min、max：最大值和最小值](#139-minmax最大值和最小值)
	- [13.10. new：分配](#1310-new分配)
	- [13.11. panic、recover：异常处理](#1311-panicrecover异常处理)
	- [13.12. print、println：格式化输出](#1312-printprintln格式化输出)
- [14. 表达式与运算符](#14-表达式与运算符)
	- [14.3. 运算符优先级](#143-运算符优先级)
	- [14.4. 算数运算符](#144-算数运算符)
	- [14.5. 比较运算符](#145-比较运算符)
	- [14.6. 逻辑运算符](#146-逻辑运算符)
	- [14.7. 地址运算符](#147-地址运算符)
	- [14.8. 接收运算符](#148-接收运算符)
	- [14.9. 常量表达式](#149-常量表达式)
	- [14.10. 类型断言](#1410-类型断言)
	- [14.11. 类型转换](#1411-类型转换)
	- [14.12. 求值顺序](#1412-求值顺序)
- [15. 语句](#15-语句)
	- [15.1. 空语句](#151-空语句)
	- [15.2. 终止语句](#152-终止语句)
	- [15.3. 标签语句](#153-标签语句)
	- [15.4. 表达式语句](#154-表达式语句)
	- [15.5. 发送语句](#155-发送语句)
	- [15.6. IncDec 语句](#156-incdec-语句)
	- [15.7. 赋值语句](#157-赋值语句)
	- [15.8. If 语句](#158-if-语句)
	- [15.9. Switch 语句](#159-switch-语句)
		- [15.9.1. Expression switch](#1591-expression-switch)
		- [15.9.2. Type switch](#1592-type-switch)
		- [15.9.3. Fallthrough](#1593-fallthrough)
	- [15.10. For 语句](#1510-for-语句)
		- [15.10.1. 单条件控制](#15101-单条件控制)
		- [15.10.2. For 子句控制](#15102-for-子句控制)
		- [15.10.3. Range 子句控制](#15103-range-子句控制)
	- [15.11. Go 语句](#1511-go-语句)
	- [15.12. Select 语句](#1512-select-语句)
	- [15.13. Return 语句](#1513-return-语句)
	- [15.14. Break 语句](#1514-break-语句)
	- [15.15. Continue 语句](#1515-continue-语句)
	- [15.16. Goto 语句](#1516-goto-语句)
	- [15.17. Defer 延迟语句](#1517-defer-延迟语句)
- [16. Error 与 Run-time Panic](#16-error-与-run-time-panic)
	- [16.1. Panic](#161-panic)
	- [16.2. Recover](#162-recover)
	- [16.3. 自定义 Error](#163-自定义-error)
...
Go 是静态类型语言，不允许混合数值类型的操作，数值类型之间不存在隐式转换。不同类型之间必须强制类型转换（例如 f`int(f64) + i` 或 `f64 + float64(i)`）。
...
- 使用内置函数 `new` 或取地复合文本值会在运行时为变量分配堆存储。
...
切片是提供其底层数组某个连续段的描述符。切片由一个指向数组的指针、段的长度及切片容量组成，与底层数组共享内存空间，`nil` 切片的长度和容量为 0。内置函数 `len` 获取切片的长度，切片长度可变。内置函数 `cap(s)` 来获取切片的容量。
...
除了特定的内置函数外，函数和方法调用以及接收操作都可以出现在语句上下文中。这些表述可以加括号。语句上下文中不允许使用以下内置函数：
...
多值赋值将表达式结果的各个元素赋值给变量列表。有两种形式。在第一种情况下，右操作数是单个多值表达式，例如函数调用、通道或映射操作或类型断言。左操作数必须与值的数量相匹配。例如，如果 `f` 是一个返回两个值的函数：
...
`switch` 表达式前面可以有一个简单语句，该语句在表达式求值之前执行。不允许多个 `case` 表达式计算同一个常量。
...
`case` 可以使用预先声明的标识符 `nil`；当 *TypeSwitchGuard* 中的表达式是 `nil` 接口值时，选择该 `case`。
...
`fallthrough` 语句置于 *ExprSwitch* 语句中的 `case` 或 `default` 分支最后一个非空语句，以允许贯穿。`fallthrough` 不能作用于最后一个分支。
...
“`range`” 子句中的范围表达式，其核心类型必须是数组、指向数组的指针、切片、字符串、映射、允许接收操作的通道或整数。与赋值一样，如果存在的话，左边的操作数必须是可寻址的或映射索引表达式；它们表示迭代变量。如果范围表达式是通道或整数，则最多允许一个迭代变量，否则最多可以有两个。如果最后一个迭代变量是空标识符，则 `range` 子句等效于没有该标识符的同一子句。
...
迭代变量可以由 “`range`” 子句使用短变量声明（`:=`）的形式来声明。在这种情况下，它们的作用域是 “`for`” 语句的块，并且每次迭代都有自己的新变量。
...
如果返回位置的作用域中存在与结果参数同名的不同实体（常量、类型或变量），编译器可能不允许 “`return`” 语句中的空表达式列表。
